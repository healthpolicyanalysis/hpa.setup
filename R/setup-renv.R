setup_renv <- function(path,
  ...
  if (!is.null(dependencies_file_path)) {
    file.copy(dependencies_file_path, file.path(path, "depencies.R"))
  }

  if (!is.null(dependencies)) {
    for (pkg in dependencies) {
      pkg_line <- glue::glue("library({pkg})")
      write(pkg_line, file.path(path, "depencies.R"), append = TRUE)
    }
  }

  withr::with_dir(path, renv::init(load = FALSE, restart = FALSE))

  file.remove(file.path(path, "depencies.R"))
